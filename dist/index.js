...
async function main() {
    ...
    server.tool("getVideoDetails", "Get detailed information about multiple YouTube videos. Returns comprehensive data including video metadata, statistics, and content details. Use this when you need complete information about specific videos.", { videoIds: z.array(z.string()) }, async ({ videoIds }) => {
        try {
            const videoPromises = videoIds.map(videoId => videoManager.getVideo({
                videoId,
    ...
    // Video search tool
    server.tool("searchVideos", "Searches for videos based on a query string. Returns a list of videos matching the search criteria, including titles, descriptions, and metadata. Use this when you need to find videos related to specific topics or keywords.", {
        query: z.string(),
    ...
    server.tool("getTranscripts", "Retrieves transcripts for multiple videos. Returns the text content of videos' captions, useful for accessibility and content analysis. Use this when you need the spoken content of multiple videos.", {
        videoIds: z.array(z.string()),
        lang: z.string().optional()
    }, async ({ videoIds, lang }) => {
        try {
            const transcriptPromises = videoIds.map(videoId => videoManager.getTranscript(videoId, lang));
    ...
    server.tool("getRelatedVideos", "Retrieves related videos for a specific video. Returns a list of videos that are similar or related to the specified video, based on YouTube's recommendation algorithm. Use this when you want to discover content similar to a particular video.", {
        videoId: z.string(),
        maxResults: z.number().optional()
    ...
    server.tool("getChannelTopVideos", "Retrieves the top videos from a specific channel. Returns a list of the most viewed or popular videos from the channel, based on view count. Use this when you want to identify the most successful content from a channel.", {
        channelId: z.string(),
        maxResults: z.number().optional()
    ...
    server.tool("getVideoEngagementRatio", "Calculates the engagement ratio for multiple videos. Returns metrics such as view count, like count, comment count, and the calculated engagement ratio for each video. Use this when you want to measure the audience interaction with videos.", { videoIds: z.array(z.string()) }, async ({ videoIds }) => {
        try {
            const engagementPromises = videoIds.map(videoId => videoManager.getVideoEngagementRatio(videoId));
    ...
    server.tool("compareVideos", "Compares multiple videos based on their statistics. Returns a comparison of view counts, like counts, comment counts, and other metrics for the specified videos. Use this when you want to analyze the performance of multiple videos side by side.", { videoIds: z.array(z.string()) }, async ({ videoIds }) => {
        try {
